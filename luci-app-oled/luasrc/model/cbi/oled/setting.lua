m = Map("oled", translate("OLED"), translate(
    "A LuCI app that helps you config your oled display (SSD1306, 0.91', 128X32) with screensavers! <br /> <br /> Any issues, please go to: ") ..
    [[<a href="https://github.com/natelol/luci-app-oled" target="_blank">luci-app-oled</a>]])
...
o = s:taboption("info", Flag, "netspeed", translate("Network speed"),
    translate("1Mbps(m/s)=1,000Kbps(k/s)=1,000,000bps(b/s)"))
o.default = 0

o = s:taboption("info", ListValue, "netsource", translate("which port to monitor"))
...
o = s:taboption("info", Value, "i2cdev", translate("I2C BUS"), translate('Choice which i2c bus you connect ssd1306'))
for k, v in pairs(nu.consume((fs.dir("/dev/")))) do
    if string.sub(v, 1, 3) == "i2c" then
        o:value("/dev/" .. v)
    end
end
o.default = "/dev/i2c-2"

o = s:taboption("info", Value, "time", translate("Display interval(s)"),
    translate('Screensaver will activate in set seconds'))
o.default = 0
