...
const gallery = read("htdocs/luci-static/resources/view/aurora/gallery.js");
const browserPresets = JSON.parse(
  read("htdocs/luci-static/resources/aurora/presets.json"),
).presets;
...
test("the browser copy of the presets matches the templates", () => {
  ...
  for (const id of PRESET_IDS) {
    ...
    assert.equal(
      Object.keys(entry.colors).length,
      62,
      `${id}: colors must stay the flat light_/dark_ map`,
    );
    assert.equal(entry.colors.light_bg, readOption(templateOf(id), "light_bg"));
  }
});
